...
def run_simulation(args):
    ...
    for i in range(start_idx, len(y)):
        ...
        # Johansen precalculado
        is_coint = coint_array[i]
        if is_coint:
...
def optimize_phase1(target, df_pivot, data_target, factors):
    ...
    print(f"[{target}] Precalculando Johansen (em paralelo)...")
    tasks_joh = [(prices_matrix, jl) for jl in joh_lookbacks]
    coint_dict = {}
    with mp.Pool(min(cores, len(joh_lookbacks))) as pool:
        for jl, arr in pool.imap_unordered(precompute_single_johansen, tasks_joh):
            coint_dict[jl] = arr
...
def main():
    ...
    df_p1.to_csv('zscore_phase1_results.csv', index=False)
    print("Fase 1 completa. Salvo em zscore_phase1_results.csv")
