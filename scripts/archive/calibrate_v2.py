...
def calibrate_target(conn, target, session_start_h=0, session_end_h=24, 
    ...
    # V2: Filtro de Correlação do Novo (>= 70% do Max Corr)
    print("  [V2] Aplicando filtro de correlação (>= 70% do Max Corr)...")
    corrs = {}
    for f in available_factors:
        label = f.replace("$N", "").lower()
        c = np.corrcoef(merged_all[label], y_ret)[0, 1]
        corrs[f] = abs(c)
    
    max_corr = max(corrs.values()) if corrs else 0
    threshold = 0.30 * max_corr
    
    # Garantir que pelo menos min_factors sobrevivam ao filtro
    sorted_factors = sorted(corrs.keys(), key=lambda x: corrs[x], reverse=True)
    if len(sorted_factors) >= min_factors:
        min_required_corr = corrs[sorted_factors[min_factors - 1]]
        threshold = min(threshold, min_required_corr)
    
    filtered_factors = []
    for f in available_factors:
        if corrs[f] >= threshold:
            filtered_factors.append(f)
            print(f"    ✅ {f:8s} | corr: {corrs[f]:.4f}")
        else:
            print(f"    ❌ {f:8s} | corr: {corrs[f]:.4f} (abaixo de {threshold:.4f})")
            
    available_factors = filtered_factors

    if len(available_factors) < min_factors:
        print(f"  ❌ Fatores restantes após filtro ({len(available_factors)}) < {min_factors}")
        return None
    ...
    for n_factors in range(min_factors, min(max_factors + 1, len(available_factors) + 1)):
        for combo in combinations(range(len(available_factors)), n_factors):
            ...
            try:
                # V2: Balanceamento Alfa usando Regressão Ridge
                # Testa diversos alfas para encontrar o melhor balanço
    ...
    weights = {}
    sigmas = {}
    # O Ridge pode não retornar o intercepto no coef_, mas sim em intercept_
    # Para consistência na formula manual score = weights * z, aplicamos normalização simples
